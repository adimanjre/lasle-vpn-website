...
const FeedbackSlider = () => {
    const[cardNumber,updateCardNumber] = useState(1)
    ...
    function moveToNext(){
        if(cardNumber < feedbackList.length ){
            console.log(sliderCardWidth)
            updateTransLateValue(prev=>prev - sliderCardWidth + 40);
            updateCardNumber(prev => prev + 1)
        }
    }
    ...
    useEffect(()=>{
        setSliderCardWidth(slideCard.current!.children[0].clientWidth);
        
    },[])
    return (
        <div className='relative overflow-hidden w-full '>
            <div ref={slideCard} className='transition-all slider-container duration-300 ease-in-out pl-5 md:pl-32 gap-10 flex flex-nowrap w-fit' style={{transform:`translate3d(${translateValue}px,0,0)`}}>
                {feedbackList.map(data => {
                    return <FeedbackCard key={data.id} classes={data.id == cardNumber?'border border-red-500':'border border-white'} imgUrl={data.imgUrl} name={data.name} address={data.address} rating={data.rating} feedback={data.feedback} />
                })}
            </div>
            <div className="flex justify-between items-center px-5 md:px-32 py-14">
                <div className='flex gap-3 items-center'>
                    {
                        feedbackList.map((data,index) => {
                            return <div key={data.id} className={`${data.id == cardNumber ? 'rounded-full bg-red-500 w-8':'rounded-full bg-gray-300  w-3'} h-3 transition-all duration-150 ease-in`}></div>
                        })
                    }
                </div>
                <div className="flex items-center gap-5">
                    ...
                    ><svg xmlns="http://www.w3.org/2000/svg" width="30" height="31" viewBox="0 0 30 31" fill="none">
                        ...
                        fill={cardNumber == feedbackList.length ? '#F53838' : '#ffffff'} />
                    </svg></button>
                </div>
            </div>
        </div>
